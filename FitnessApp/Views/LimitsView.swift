...
struct LimitsView: View {
    ...
    @State private var editing: LimitField? = nil
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Limiti")
                                ...
                                .foregroundColor(.txt)
                            Text("Tocca un valore per modificarlo")
                                .font(.system(size: 13))
...
struct LimitRow: View {
    ...
    var displayValue: String {
        unit == "kcal" || unit == "" ? "\(Int(value))\(unit.isEmpty ? "" : " \(unit)")" : "\(Int(value)) \(unit)"
    }
    ...
    var body: some View {
        Button(action: onTap) {
            ...
            .overlay(alignment: .top) {
                if !last {
                    Rectangle().fill(Color.card2).frame(height: 1).padding(.leading, 18)
                }
            }
        }
        .buttonStyle(.plain)
...
struct LimitEditSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Modifica limite")
        ...
        .onAppear {
            input = field.isInt ? "\(Int(field.current))" : field.current.formatted1
        }
    }
}
